module.exports = {
    ayuda: `Personalizas completamente la salida 3. ({usuario} Para el usuario y {servidor} Para el nombre del servidor)`,
    ejemplo: "personalizar-salida3 Se fue {usuario}, Hasta luego ,blanco,https://i.imgur.com/Hz5xag2.jpg",
...
*/
    ...
    let dbselect = db.prepare(`SELECT salida3_fondo FROM personalizar_comandos WHERE idserver = ${message.guild.id}`).get();

console.log(dbselect)

    if(!dbselect || dbselect.entrada3_fondo == null || dbselect.entrada3_fondo == undefined){
      db.prepare(`INSERT INTO personalizar_comandos(idserver, salida3_fondo, salida3_color, salida3_titulo ,salida3_descripcion) VALUES(${message.guild.id}, '${fondo}', '${color}', '${textoTitulo}', '${textoDesc}')`).run();
    }else{
      db.prepare(`UPDATE personalizar_comandos SET salida3_fondo = '${fondo}' WHERE idserver = ${message.guild.id}`).run();
      db.prepare(`UPDATE personalizar_comandos SET salida3_color = '${color}' WHERE idserver = ${message.guild.id}`).run();
      db.prepare(`UPDATE personalizar_comandos SET salida3_titulo = '${textoTitulo}' WHERE idserver = ${message.guild.id}`).run();
