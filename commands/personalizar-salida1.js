module.exports = {
  ...
  embednoper.setDescription("🚫 || No tienes el permiso ``ADMINISTRATOR`` para utilizar este comando.")    
    ...
    let dbselect = db.prepare(`SELECT texto_salida1 FROM personalizar_comandos WHERE idserver = ${message.guild.id}`).get();

    if(!dbselect){
      db.prepare(`INSERT INTO personalizar_comandos(idserver, texto_salida1) VALUES(${message.guild.id}, '${texto}')`).run();
    }else{
      db.prepare(`UPDATE personalizar_comandos SET texto_salida1 = '${texto}' WHERE idserver = ${message.guild.id}`).run();
    };

    const embed1 = new Discord.MessageEmbed() 
    embed1.setColor("GREEN")
    embed1.setDescription(`<:tick:613820461675053066> || Se ha modificado el mensaje de la salida1.`)
